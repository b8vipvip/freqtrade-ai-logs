...
class MultiCoin_AI_Strategy_20260529_040545_v030(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["directional_consistency_12"] = (
            (dataframe["close"] > dataframe["close"].shift(1))
            .rolling(window=12, min_periods=12)
            .mean()
        )
        dataframe["local_drawdown_48"] = (
            dataframe["close"] / dataframe["close"].rolling(window=48, min_periods=48).max()
        ) - 1.0
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if pair.startswith("BTC/"):
            btc_quality_filter = (
                (dataframe["directional_consistency_12"] >= 0.50) &
                (dataframe["local_drawdown_48"] > -0.04) &
                (dataframe["close"] > dataframe["ema_50"] * 0.995)
            )
            long_condition = long_condition & btc_quality_filter
